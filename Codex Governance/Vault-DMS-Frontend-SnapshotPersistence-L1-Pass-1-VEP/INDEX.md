...
|---|---|---|---|---|
| `dmsClient` module (`lib/dmsClient.ts`; ACTIVE, modify) | Exported API adds `setDmsPrincipal(id: string): void` (binds the cache to the authenticated principal + re-hydrates from that principal's namespace). Prior exports UNCHANGED: `getCachedSites` / `getCachedTree` / `isNodeExpanded` / `setNodeExpanded` / `clearDmsCache`; `listDmsSites` / `getDmsTree` signatures UNCHANGED. Internal: sessionStorage keys namespaced `vault-dms:v1:<principal>:*`; ss I/O gated on a bound principal; write/clear on the success paths, `setNodeExpanded`, `clearDmsCache` (guarded). | VA-D1 (no render change) | `dms_list_sites` §2.6 / `dms_tree` §2.2 (unchanged) | PROCEED |
| `DmsBrowser` (`DmsBrowser.tsx`; ACTIVE, modify) | Prop interface UNCHANGED (`DmsBrowserProps` incl. `active?`). Internal: on mount resolve the OID from the token (`oidFromToken`, cache-keying only) → `setDmsPrincipal`, and gate the tree render (`principalReady`) until bound so the snapshot seeds under the correct principal. No render/visual change. | VA-D1 | via getters + token | PROCEED |
