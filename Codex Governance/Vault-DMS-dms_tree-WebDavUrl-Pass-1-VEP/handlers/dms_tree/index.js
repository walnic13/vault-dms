    ...
    for (const item of items) {
      ...
      if (item.folder && typeof item.folder === "object") {
        ...
        children.push({
          ...
          type: "file",
          mime_type: mimeType,
          // ALLOWED DELTA (web_dav_url): the file's WebDAV direct path, projected from the SAME
          // /children DriveItem already fetched — no new Graph call, no new endpoint. The SharePoint
          // web_url is a Doc.aspx sharing/viewer URL the desktop Office URI scheme (ms-excel:ofe|u|)
          // cannot open; web_dav_url is the direct path the desktop app opens. Files only — open-in-app
          // is a file affordance (folders do not carry it).
