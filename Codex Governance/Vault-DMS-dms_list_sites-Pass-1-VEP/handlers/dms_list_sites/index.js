...
module.exports = async function (context, req) {
  ...
  try {
    // Stateless tenant-wide site ENUMERATION: no q parameter — enumerate every site the
    // signed-in user can access via Graph site search with the wildcard term "*", paginating
    // through @odata.nextLink so the full tenant DMS is returned (not just the first page).
    // No database, no registry — SharePoint is the system of record. The per-row filter +
    // /drive permission probe mirror reporting_search_clients' Graph path; the while(nextUrl)
    // pagination loop mirrors reporting_dms_tree (Walter-authorized composite primary reference).
    const graphToken = await exchangeGraphToken(oboInput.token);

    const sites = [];
    const seenSiteIds = new Set();

    let nextUrl = `https://graph.microsoft.com/v1.0/sites?search=${encodeURIComponent("*")}`;

    while (nextUrl) {
      const page = await graphGetJson(nextUrl, graphToken);

      const graphRows = Array.isArray(page.value) ? page.value : [];
    ...
    context.log.error("dms_list_sites failed", err);
    ...
    return send(
      context,
      500,
      errorBody("INTERNAL_SERVER_ERROR", "Failed to list DMS sites.", 500)
    );
  }
};
