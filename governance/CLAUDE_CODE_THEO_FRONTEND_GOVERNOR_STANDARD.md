...
3. **No browser storage — scoped exception.** 1A state is React/in-memory; persistence is 1B. **Exception — DMS Snapshot Storage Exception (Walter-authorized 2026-07-19):** the productionised DMS remote (`vault-dms`) MAY use `sessionStorage` solely to persist its non-sensitive browse snapshot (SharePoint site/folder listing metadata — names, item ids, and web_url/web_dav_url — plus the expanded-node set) and, when introduced, the Graph delta cursor, to give instant paint across reload for a live-mirror UX. Constraints (all BINDING): `sessionStorage` only (per-tab; cleared on tab close); metadata + delta cursor ONLY — never file content, access tokens, secrets, or any credential; employees-only origin; ALWAYS paired with revalidation (the persisted snapshot is an instant-paint seed, never authoritative-stale); cleared by `clearDmsCache()`; all writes try/guarded. All other browser storage, and any other repo/surface, remains prohibited under this rule.
